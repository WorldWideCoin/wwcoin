...
// Copyright (c) 2014 WWCoin developers
...
int64 CTransaction::nMinTxFee = 100;  // 0.001 WWC Override with -mintxfee
/** Fees smaller than this are considered zero fee (for relaying) */
...
const string strMessageMagic = "WWCoin Signed Message:\n";
...
static const int64 nPremine = 50 * 1000000000000 * COIN; //50 trillion
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 500 * COIN;
    } else if (nHeight > 4500 && nHeight <= 6000) {
        nSubsidy = 1000 * COIN;
    } else {
...
static const int64 nTargetTimespan = 60 * 60 * 24; // 1 day retargeting
static const int64 nTargetSpacing = 60 * 5; //1 block per 5 minutes
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // 2880 blocks

static const int64 nAveragingInterval = nInterval; // 60 blocks
static const int64 nAveragingTargetTimespan = nAveragingInterval * nTargetSpacing; //1 hour
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    unsigned int nGenesisProofOfWorkLimit = Params().GenesisProofOfWorkLimit().GetCompact();

    // Genesis block
    if (pindexLast == NULL)
        return nGenesisProofOfWorkLimit;
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits)
{
    ...
    CBigNum bnProofOfWork =  (hash != Params().HashGenesisBlock())? Params().ProofOfWorkLimit() : Params().GenesisProofOfWorkLimit();

    // Check range
    if (bnTarget <= 0 || bnTarget > bnProofOfWork)
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WWCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WWCoinMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running WWCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
